...
export async function runGenerateBarcode(
  args: {
    text: string;
  ...
  const base64 = await apitierPost(
    `${config.baseUrls.barcode}/generate/${format}`,
    body,
    config.keys.barcode!,
    true
  );
...
export async function runGenerateQrCode(
  args: {
    text: string;
    width?: number;
    height?: number;
  ...
  const base64 = await apitierPost(
    `${config.baseUrls.barcode}/generate/qrcode`,
    body,
    config.keys.barcode!,
    true
  );
